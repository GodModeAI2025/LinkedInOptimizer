...
Ein Skill zur professionellen Analyse und Optimierung von LinkedIn-Profilen. Entwickelt für
Berater, Agenturen und Freelancer, die Kunden strategisch als Thought Leader positionieren und auf
das LinkedIn Top Voice Badge vorbereiten.

Acht Phasen, von der Datenerhebung bis zur Übergabe. Jede Zahl im Skill hat eine Quelle oder ist
als Erfahrungswert gekennzeichnet; was sich nicht belegen ließ, ist entfernt und gegen die Rückkehr
gesperrt.

Für den laufenden Betrieb danach, also Wochensystem, Content-Kalender, Community und
Monetarisierung, gibt es den Schwester-Skill
[linkedin-community-builder](https://github.com/GodModeAI2025/LinkedIn-Orchestrator).
...
> „Erstelle mir ein LinkedIn-Banner."

> „Wie stehe ich gegen meine Wettbewerber da?"

> „Bin ich reif für ein Top Voice Badge?"

Der Skill führt durch die Phasen und fragt nach, was er nicht sehen kann.

## Installation

| Weg | Befehl |
|-----|--------|
| Claude Code (CLI, Desktop, IDE) | `/plugin marketplace add GodModeAI2025/LinkedInOptimizer`, dann `/plugin install linkedin-profil-optimierung@linkedin-optimizer` |
| claude.ai (Web) | Skills in der Seitenleiste → **Add from GitHub** → `GodModeAI2025/LinkedInOptimizer` |
| Release-Artefakt | siehe unten |
| Selbst gebaut | `python scripts/build_skill_package.py dist/linkedin-optimizer.skill` |

Der Skill liegt im Wurzelverzeichnis, nicht unter `skills/`. Claude Code lädt eine SKILL.md im
Plugin-Wurzelverzeichnis; `.claude-plugin/plugin.json` sagt es zusätzlich mit `"skills": ["."]`.
...
| Die Profildaten bleiben leer | LinkedIn hat das Markup geändert oder die Chrome-Sitzung ist nicht angemeldet. Die Selektoren stehen in `references/PHASE-1-discovery.md`. |
| `create_banner.py` bricht ab | Pillow ist älter als 10.1, siehe requirements.txt. |
| `generate_report.js` wirft ENOENT | Der Ausgabepfad ist auf die claude.ai-Sandbox verdrahtet. Letzte Zeile anpassen. |

<details>
<summary><b>Für Entwickler: Struktur, Tests, Konventionen</b></summary>
...
├── SECURITY.md                 # Bedrohungsmodell, Vertrauensgrenzen, bekannte Lücken
├── .claude-plugin/
...
└── .github/workflows/
    ├── ci.yml                  # Syntax, Banner, Versionen, Trigger, Quellen, Hooks, Eval, Paket
    ├── dependabot.yml          # Hält die Actions monatlich aktuell
